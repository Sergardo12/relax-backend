...
@Injectable()
export class ObtenerHorarioColaboradorUseCase {
  constructor(
    @Inject(HORARIO_COLABORADOR_REPOSITORY_TOKEN)
    private readonly horarioColaboradorRepository: HorarioColaboradorRepository,
  ) {}

  async ejecutar(id: string): Promise<Result<HorarioColaborador>> {
    return await this.horarioColaboradorRepository.findById(id);
  }
}
