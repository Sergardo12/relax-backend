...
@Injectable()
export class ListarSesionesTratamientoUseCase {
  constructor(
    @Inject(SESION_TRATAMIENTO_REPOSITORY_TOKEN)
    private readonly sesionRepository: SesionTratamientoRepository,
  ) {}

  async execute(idTratamiento: string): Promise<Result<SesionTratamiento[]>> {
    return await this.sesionRepository.findByTratamientoId(idTratamiento);
  }
}